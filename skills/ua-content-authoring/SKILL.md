...
14. This repo is docs-only; do not add game pages or game JS/CSS here (game lives in `C:\Git\UniversalArena-Web`).
15. Export actions (page PDF/MD/TXT + all-pages ZIP with format/character-combine options and `html/`, `md/`, `txt/`) live in `docs/overrides/main.html` and `docs/javascripts/print.js`; keep exports free of permalinks, URLs, and images when editing them.
16. Run `mkdocs build --strict` or `mkdocs serve` when asked to validate.
